...
class CreateAccount(object):
    ...
    def status(self):
        ...
        Status of your account. Possible values are:   - enabled: Account is ready to be used   - suspended: Account has been temporarily suspended   - disabled: Account has been permanently disabled and             cannot be revived. All resources allocated             to the subaccount like phonenumbers are also             deleted.   `suspended` and `disabled` accounts cannot use Karix API. They will receive   `403 Forbidden` response.   # noqa: E501
